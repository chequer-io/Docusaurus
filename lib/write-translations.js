...
function execute(_language) {
  ...
  if (fs.existsSync(`${CWD}/i18n/${language}.json`)) {
    currentTranslations = JSON.parse(
      fs.readFileSync(`${CWD}/i18n/${language}.json`, 'utf8')
    );
  }
  ...
  let files = glob.sync(CWD + (language === 'en' ? '/../' + readMetadata.getDocsPath() :
    '/translated_docs') + '/**');
  files.forEach(file => {
  ...
  files = glob.sync(CWD + `/pages/${language}/**`);
  files.forEach(file => {
    const extension = path.extname(file);
    if (extension === '.js') {
      ...
      traverse(ast, {
        enter(path) {
          if (
            ...
            translations['pages-strings'][text + '|' + description] =
              text;
          }
        },
      });
...
function executeTranslatedDocs() {
  const env = require('./server/env.js');

  // i18n write for non-english docs
  const enabledLanguages = env.translation
    .enabledLanguages()
    .map(language => language.tag);

  const regexSubFolder = /translated_docs\/(.*?)\/.*/;
  let files = glob.sync(CWD + '/translated_docs/**');
  files.forEach(file => {
    let language = 'en';
